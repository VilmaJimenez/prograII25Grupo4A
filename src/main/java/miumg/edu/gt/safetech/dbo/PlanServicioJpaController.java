...
public class PlanServicioJpaController implements Serializable {
    ...
    public void create(PlanServicio planServicio) {
        if (planServicio.getServicioList() == null) {
            planServicio.setServicioList(new ArrayList<Servicio>());
        }
        ...
        try {
            ...
            for (Servicio servicioListServicioToAttach : planServicio.getServicioList()) {
                servicioListServicioToAttach = em.getReference(servicioListServicioToAttach.getClass(), servicioListServicioToAttach.getIdServicio());
                attachedServicioList.add(servicioListServicioToAttach);
            }
            planServicio.setServicioList(attachedServicioList);
            ...
            for (Servicio servicioListServicio : planServicio.getServicioList()) {
                PlanServicio oldIdPlanOfServicioListServicio = servicioListServicio.getIdPlan();
                servicioListServicio.setIdPlan(planServicio);
                servicioListServicio = em.merge(servicioListServicio);
                if (oldIdPlanOfServicioListServicio != null) {
                    oldIdPlanOfServicioListServicio.getServicioList().remove(servicioListServicio);
                    oldIdPlanOfServicioListServicio = em.merge(oldIdPlanOfServicioListServicio);
                }
            }
    ...
    public void edit(PlanServicio planServicio) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<Servicio> servicioListOld = persistentPlanServicio.getServicioList();
            List<Servicio> servicioListNew = planServicio.getServicioList();
            ...
            for (Servicio servicioListNewServicioToAttach : servicioListNew) {
                servicioListNewServicioToAttach = em.getReference(servicioListNewServicioToAttach.getClass(), servicioListNewServicioToAttach.getIdServicio());
                attachedServicioListNew.add(servicioListNewServicioToAttach);
            }
            servicioListNew = attachedServicioListNew;
            planServicio.setServicioList(servicioListNew);
            ...
            for (Servicio servicioListOldServicio : servicioListOld) {
                if (!servicioListNew.contains(servicioListOldServicio)) {
                    servicioListOldServicio.setIdPlan(null);
            ...
            for (Servicio servicioListNewServicio : servicioListNew) {
                if (!servicioListOld.contains(servicioListNewServicio)) {
                    PlanServicio oldIdPlanOfServicioListNewServicio = servicioListNewServicio.getIdPlan();
                    servicioListNewServicio.setIdPlan(planServicio);
                    servicioListNewServicio = em.merge(servicioListNewServicio);
                    if (oldIdPlanOfServicioListNewServicio != null && !oldIdPlanOfServicioListNewServicio.equals(planServicio)) {
                        oldIdPlanOfServicioListNewServicio.getServicioList().remove(servicioListNewServicio);
                        oldIdPlanOfServicioListNewServicio = em.merge(oldIdPlanOfServicioListNewServicio);
                    }
                }
            }
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            List<Servicio> servicioList = planServicio.getServicioList();
            for (Servicio servicioListServicio : servicioList) {
                servicioListServicio.setIdPlan(null);
                servicioListServicio = em.merge(servicioListServicio);
            }
